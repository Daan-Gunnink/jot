...
import { ref, watch, nextTick, onMounted, onBeforeUnmount, type PropType, type CSSProperties } from 'vue';
...
const props = defineProps({
    ...
    style: {
        ...
        default: () => ({ position: 'absolute', /* Add default positioning if needed */ })
    }
});
...
const handleGlobalKeyDown = (event: KeyboardEvent) => {
    ...
    console.log('Global listener sees:', event.key, 'Current Index:', selectedIndex.value);
...
watch(() => props.items, (newItems, oldItems) => {
    ...
    if (newItems.length !== oldItems?.length || newItems[0]?.id !== oldItems?.[0]?.id) {
        selectedIndex.value = 0;
        if (suggestionListRef.value) {
            suggestionListRef.value.scrollTop = 0;
        }
    }
}, { deep: false }); // Don't need deep watch here
