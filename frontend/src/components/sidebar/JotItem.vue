<template>
    <ContextMenuRoot class="w-full h-full">
        <ContextMenuTrigger as-child class="">
            <RouterLink :key="props.jot.id" :to="`/jot/${props.jot.id}`" class="p-4 w-full hover:bg-base-300"
    ...
    <AlertModal :open="displayDeleteConfirmation" :title="`Delete ${props.jot.title}?`"
        ...
        actionDescription="Delete" @action="handleDelete" @cancel="displayDeleteConfirmation = false" />
</template>
